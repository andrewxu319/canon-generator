...
void write_file(const mx::api::ScoreData score, const std::string& path)
{
    using namespace mx::api;
    const auto qticks = 4;

    // create a score
    auto fake_score = ScoreData{};
    fake_score.workTitle = "Mx Example";
    fake_score.composer = "Matthew James Briggs";
    fake_score.copyright = "Copyright (c) 2019";
    fake_score.ticksPerQuarter = qticks;

    // create a part
    fake_score.parts.emplace_back(PartData{});
    auto& part = fake_score.parts.back();
    ...
    // the document manager is the liaison between our fake_score data and the MusicXML DOM.
    // it completely hides the MusicXML DOM from us when using mx::api
    auto& mgr = DocumentManager::getInstance();
    const auto documentID = mgr.createFromScore(fake_score);
    ...
    // write to a file
    mgr.writeToFile(documentID, "./EXAMPLE_EXAMPLE.musicxml");
...
void write_file(const mx::api::ScoreData score, const std::string& path) {
    ...
    // write to a file
    mgr.writeToFile(documentID, path);
